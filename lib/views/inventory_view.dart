...
class _InventoryViewState extends State<InventoryView> {
  ...
  String _selectedPricingTab = 'Quantity-Based';
          ...
          children: [
            Column(
              children: [
                        ...
                        children: [
                          const Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 4),
                              Text(
                                'Manage products, stock quantities, pricing types, and categories.',
                ...
                // Pricing Type Segmented Tabs
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 32),
                  child: Row(
                    children: [
                      _buildPricingTab('Quantity-Based'),
                      const SizedBox(width: 12),
                      _buildPricingTab('Weight-Based'),
                    ],
                  ),
                ),
  ...
  Widget _buildPricingTab(String title) {
    final isSelected = _selectedPricingTab == title;
    return InkWell(
      onTap: () => setState(() => _selectedPricingTab = title),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 10),
        decoration: BoxDecoration(
          color: isSelected ? BoutiqueColors.accent : BoutiqueColors.bgCard,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: isSelected ? BoutiqueColors.accent : BoutiqueColors.border),
        ),
        child: Text(
          title,
          style: TextStyle(
            fontSize: 13,
            fontWeight: FontWeight.bold,
            color: isSelected ? Colors.white : BoutiqueColors.textSecondary,
          ),
        ),
      ),
    );
  ...
  Widget _buildTableRow(Product p) {
    final isLowStock = p.pricingType == 'Quantity-Based' && p.quantity < 5;
    return ListTile(
      ...
      leading: Container(
        ...
        child: const Center(
          child: Icon(Icons.checkroom_rounded, color: BoutiqueColors.accent, size: 22),
        ),
      ),
      title: Row(
        children: [
      ...
      subtitle: Text(
        p.pricingType == 'Quantity-Based'
            ? '${p.category} • ${p.material} • Stock: ${p.quantity} ${p.unit}'
            : '${p.category} • ${p.material} • Gross Wt: ${p.grossWeight}g',
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Center(
              child: Icon(Icons.checkroom_rounded, size: 48, color: BoutiqueColors.accent),
            ),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow('Pricing Type', p.pricingType),
          _buildDetailRow('Material', p.material.isNotEmpty ? p.material : 'N/A'),
